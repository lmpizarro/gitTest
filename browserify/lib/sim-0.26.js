...
Sim.prototype.sendMessage=function() {
    var a=this.source,b=this.msg,c=this.data,d=a.sim;
    if(c)if(c instanceof Array)for(f=c.length-1; f>=0; f--)e=c[f],e!==a&&e.onMessage&&e.onMessage.call(e,a,b);
        else c.onMessage&&c.onMessage.call(c,a,b);
    else for(var f=d.entities.length-1; f>=0; f--) {
            var e=d.entities[f];
...
Sim.prototype.addEntity=function(a) {
    ...
    a.send=function(a,b,c) {b=new Sim.Request(this.sim,this.time(),this.time()+b); b.source=this; b.msg=a; b.data=c; b.deliver=this.sim.sendMessage; this.sim.queue.insert(b)},a.log=function(a) {
        this.sim.log(a,this)
    };
    var b=function(a) {
        function b() {} if(a==null)throw TypeError();
...
Sim.Facility.prototype.useProcessorSharingSchedule=function(a,b) {
    var c=a.entity.time(),d=this.queue.length,f=b?(d+1)/d:(d-1)/d,e=[];
    ...
    if(b)g=new Sim.Request(this,c,c+a.duration*(d+1)),g.ro=a,g.source=this,g.deliver=this.useProcessorSharingCallback,
        e.push(g),a.entity.sim.queue.insert(g);
    this.queue=e;
...
Sim.Buffer.prototype.progressGetQueue=function() {
    for(var a; a=this.getQueue.top();)if(a.cancelled)this.getQueue.shift(a.entity.time());
...
Sim.Buffer.prototype.progressPutQueue=function() {
    for(var a; a=this.putQueue.top();)if(a.cancelled)this.putQueue.shift(a.entity.time());
...
Sim.Store.prototype.get=function(a,b) {
    if(this.getQueue.empty()&&this.current()>0) {
        var c=!1,d;
        if(a)for(var f=0; f<this.objects.length; f++) {
...
Sim.Store.prototype.progressGetQueue=function() {
    for(var a; a=this.getQueue.top();)if(a.cancelled)this.getQueue.shift(a.entity.time());
            ...
            var b=a.filter,c=!1,d;
            if(b)for(var f=0; f<this.objects.length; f++) {
...
Sim.Request.prototype.unlessEvent=function(a,b,c,d) {
    ...
    else if(a instanceof Array)for(var e=0; e<a.length; e++)f=this._addRequest(0,b,c,d),f.msg=a[e],a[e].addWaitList(f);
    return this
};
...
Sim.PQueue.prototype.greater=function(a,b) {
    return a.deliverAt>b.deliverAt?!0:a.deliverAt==b.deliverAt?a.order>b.order:!1
};
...
Sim.PQueue.prototype.remove=function() {
    ...
    if(!(b<=0)) {
        ...
        for(var d=0,f=a[d]; d<Math.floor(b/2);) {
            var e=2*d+1,h=2*d+2,e=h<b&&!this.greater(a[h],a[e])?h:e;
...
Sim.DataSeries.prototype.reset=function() {
    ...
    this.Min=Infinity;
    this.Sum=0;
    if(this.histogram)for(var a=0; a<this.histogram.length; a++)this.histogram[a]=0
        };
...
Sim.DataSeries.prototype.record=function(a,b) {
    ...
    this.Sum+=a;
    this.Count++;
    this.histogram&&(a<this.hLower?this.histogram[0]+=c:a>this.hUpper?this.histogram[this.histogram.length-1]+=c:this.histogram[Math.floor((a-this.hLower)/this.hBucketSize)+1]+=c);
    this.W+=c;
    if(this.W!==0) {
        var d=this.A;
...
Random.prototype.init_by_array=function(a,b) {
    ...
    for(f=this.N-1; f; f--)e=this.mt[c-1]^this.mt[c-1]>>>30,this.mt[c]=(this.mt[c]^(((e&4294901760)>>>16)*1566083941<<16)+(e&65535)*1566083941)-c,this.mt[c]>>>=0,c++,c>=this.N&&(this.mt[0]=
...
Random.prototype.gamma=function(a,b) {
    if(a>1)for(var c=Math.sqrt(2*a-1),d=a-this.LOG4,f=a+c;;) {
            var e=this.random();
            if(!(e<1.0E-7||j>0.9999999)) {
                var h=1-this.random(),i=Math.log(e/(1-e))/c,g=a*Math.exp(i),e=e*e*h,i=d+f*i-g;
                if(i+this.SG_MAGICCONST-4.5*e>=0||i>=Math.log(e))return g*b
                }
        }
    else if(a==1) {
        ...
        for(;;)if(j=this.random(),g=(Math.E+a)/Math.E,j*=g,g=j<=1?Math.pow(j,1/a):-Math.log((g-j)/a),e=this.random(),j>1) {
                if(e<=Math.pow(g,
                               a-1))break
                }
...
Random.prototype.normal=function(a,b) {
    ...
    this.lastNormal=NaN;
    if(!c) {
        var d=this.random()*2*Math.PI,f=Math.sqrt(-2*Math.log(1-this.random())),c=Math.cos(d)*f;
